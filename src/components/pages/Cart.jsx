                        ...
                        <tbody>
                            {cartItems.products?.map((item, index) => (
                                <tr key={item.product._id}>
                                    ...
                                    <td className='flex align-center justify-center'><img src={item.product?.images[0]?.img} width="70px" alt="" /></td>
                                    <td>${item.product?.price}</td>
                                    <td>${item.amount}</td>
                                    <td>
                                        <form action="">
                                            <input type="number" onChange={(e) => updateCartItems(item.product._id, e.target.value)} min={1} value={item.quantity} name="" id="" />
                                        </form>
                                    </td>
                                    <td>
                                        <button type="submit" onClick={() => removeCartItems(item.product._id)}><i className="fa-solid fa-xmark"></i></button>
                                    </td>
                                </tr>
                            ))}    
